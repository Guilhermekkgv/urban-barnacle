...
if Linux.LanguageDropdown and Linux.LanguageDropdown.SetOptions then
...
if config.SearchDropdown ~= nil then
Linux.SearchDropdown = config.SearchDropdown
end
...
local ExpandButton = Linux.Instance("ImageButton", {
Parent = TopBar,
BackgroundTransparency = 1,
Size = UDim2.new(0, 16, 0, 16),
...
local expandedSize = UDim2.new(0, workspace.CurrentCamera.ViewportSize.X * 0.9, 0, originalSize.Y.Offset)
...
globalConnectionManager:addConnection(ExpandButton.MouseEnter:Connect(function()
local tween = TweenService:Create(ExpandButton, TweenInfo.new(0.2), { ImageColor3 = Color3.fromRGB(255, 255, 255) })
globalConnectionManager:addTween(tween)
tween:Play()
end))
globalConnectionManager:addConnection(ExpandButton.MouseLeave:Connect(function()
local tween = TweenService:Create(ExpandButton, TweenInfo.new(0.2), { ImageColor3 = Color3.fromRGB(180, 180, 190) })
globalConnectionManager:addTween(tween)
tween:Play()
end))
globalConnectionManager:addConnection(ExpandButton.MouseButton1Click:Connect(function()
isExpanded = not isExpanded
if isExpanded then
expandedSize = UDim2.new(0, workspace.CurrentCamera.ViewportSize.X * 0.9, 0, Main.Size.Y.Offset)
local tween1 = TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quad), { Size = expandedSize })
local tween2 = TweenService:Create(Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
Size = UDim2.new(0, expandedSize.X.Offset + 120, 0, expandedSize.Y.Offset + 116)
})
globalConnectionManager:addTween(tween1)
globalConnectionManager:addTween(tween2)
tween1:Play()
tween2:Play()
Main.Position = UDim2.new(0.5, -expandedSize.X.Offset / 2, 0.5, -expandedSize.Y.Offset / 2)
else
local tween1 = TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quad), { Size = originalSize })
local tween2 = TweenService:Create(Shadow, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
Size = UDim2.new(0, originalSize.X.Offset + 120, 0, originalSize.Y.Offset + 116)
...
local tween = TweenService:Create(CollapseArrow, TweenInfo.new(0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Rotation = IsCollapsed and 0 or 90 })
globalConnectionManager:addTween(tween)
tween:Play()
...
Element = element,
TabName = Tabs[tabIndex].Name,
GetValue = function() return Checkbox:GetAttribute("State") end,
SetValue = SetValue
})
globalConnectionManager:elementLoaded()
CheckEmptyState()
...
local SearchBox = nil
local SearchText = ""
if Linux.SearchDropdown then
SearchBox = Linux.Instance("TextBox", {
Parent = DropFrame,
...
BackgroundColor3 = Linux.FrameColor,
BackgroundTransparency = 1,
Size = Linux.SearchDropdown and UDim2.new(1, 0, 1, -40) or UDim2.new(1, 0, 1, 0),
Position = Linux.SearchDropdown and UDim2.new(0, 0, 0, 40) or UDim2.new(0, 0, 0, 0),
CanvasSize = UDim2.new(0, 0, 0, 0),
AutomaticCanvasSize = Enum.AutomaticSize.Y,
ScrollBarThickness = 0,
ZIndex = 2,
...
local maxHeight = Linux.SearchDropdown and 190 or 150
...
if Linux.SearchDropdown and SearchText ~= "" then
DropFrame.Size = UDim2.new(1, 0, 0, finalHeight)
else
...
if string.find(string.lower(tostring(opt)), string.lower(SearchText)) then
...
if #filteredOptions == 0 and Linux.SearchDropdown and SearchText ~= "" then
local NoResultsLabel = Linux.Instance("TextLabel", {
Parent = OptionsHolder,
BackgroundTransparency = 1,
...
NoResultsLabel.TextTransparency = 1
local fadeInTween = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local tween = TweenService:Create(NoResultsLabel, fadeInTween, { TextTransparency = 0 })
globalConnectionManager:addTween(tween)
tween:Play()
end
...
if Linux.SearchDropdown and SearchBox then
globalConnectionManager:addConnection(SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
...
configNameInput.SetValue(Linux.SavedConfigs)
Linux.CurrentConfig = Linux.SavedConfigs
print("[INFO] Switched to config '" .. Linux.SavedConfigs .. "'")
else
